...
def test_read_file_names_emr_s3():
    ...
    arr = s.RUN_DATE.split("-")
    test_date = os.path.join(arr[0], arr[1])
    file_name = s.FILE_NAME_TEMPLATE.replace("yyyy", arr[0]).replace("mm", arr[1])
    expected_raw = os.path.join(s.S3_BUCKET, s.S3_RAW_KEY, test_date, file_name)
    expected_processed = os.path.join(s.S3_BUCKET, s.S3_PRCSD_KEY, test_date, file_name)
...
def test_read_file_names_local():
    ...
    expected_raw = os.path.join(s.LOCAL_RAW_DATA_PATH, test_date, file_name)
    expected_processed = os.path.join(s.LOCAL_PRCSD_DATA_PATH, test_date, file_name)
...
def test_read_file_names_local_s3():
    ...
    expected_raw = os.path.join("s3a://" , s.S3_BUCKET_SIMPLE, s.S3_RAW_KEY, test_date, file_name)
    expected_processed = os.path.join("s3a://" , s.S3_BUCKET_SIMPLE, s.S3_PRCSD_KEY, test_date, file_name)
...
def test_read_extras_names_local():
    ...
    test_date = os.path.join(arr[0], arr[1])
    file_name = s.FILE_NAME_TEMPLATE.replace("yyyy", arr[0]).replace("mm", arr[1])
    expected_processed = os.path.join(s.LOCAL_PRCSD_DATA_PATH, test_date , s.LOCAL_EXTRAS ,file_name)
